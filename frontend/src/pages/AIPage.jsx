            ...
            {messages.length === 0 && (
              <div className="max-w-3xl mx-auto space-y-6">
                <div className="text-center">
                  <div className={`inline-flex h-16 w-16 items-center justify-center rounded-2xl ${isStudent ? 'bg-emerald-500/10 text-emerald-500' : 'bg-accent/10 text-accent'} mb-4`}>
                    {isStudent ? <GraduationCap className="h-8 w-8" /> : <Sparkles className="h-8 w-8" />}
                  </div>
                  <h2 className="font-display text-2xl font-semibold">
                    {isStudent ? `What shall we study today, ${user?.fullName?.split(' ')[0]}?` : `How can I help you today, ${user?.fullName?.split(' ')[0]}?`}
                  </h2>
                  <p className="text-muted-foreground mt-1">
                    {isStudent ? 'Ask me about your class tasks, homework, active projects, or any subject questions.' : 'Ask me to create homework for your class, check who has submitted homework, or draft class updates.'}
                  </p>
                </div>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {activePrompts.map((q, i) => (
                    <button key={i} onClick={() => send(q.prompt)} className="text-left rounded-lg border border-border p-4 hover:border-accent transition-colors" data-testid={`ai-prompt-${i}`}>
                      <div className="flex items-center gap-2">
                        <q.icon className={`h-4 w-4 ${isStudent ? 'text-emerald-500' : 'text-accent'}`} />
